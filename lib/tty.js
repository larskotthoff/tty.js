...
function Server(conf) {
  ...
  passport.use(new GitHubStrategy({
      ...
      callbackURL: 'http://localhost:8000/login/return'
    },
    ...
    return sessionStore.get(sessionId, function(err, session) {
        if(session && session.passport.user) {
          return self.handleConnection(socket,
              crypto.createHash('md5').update(session.passport.user.displayName).digest('hex'));
        }
    });
  });
};

Server.prototype.handleConnection = function(socket, user) {
  var session = new Session(this, socket, user);
...
function Session(server, socket, user) {
  this.server = server;
  this.socket = socket;
  this.terms = {};
