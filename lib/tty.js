...
function Server(conf) {
  ...
  passport.use(new GitHubStrategy({
    ...
    function(identifier, dontknow, profile, done) {
...
Server.prototype.initMiddleware = function() {
  ...
  this.app.use(express.session({ secret: conf.cookieSecret, store: this.sessionStore }));
  ...
  this.app.get('/login/return',
      passport.authenticate('github',
          { successRedirect: '/',
            failureRedirect: '/login' }));

  this.use("/", ensureAuthenticated);
};

function ensureAuthenticated(req, res) {
  if(req.isAuthenticated()) {
      return express.static(__dirname + '/../static')(req, res);
  }
  res.redirect('/login');
}
...
Server.prototype.initIO = function() {
  ...
  io.set('authorization', function(data, callback) {
    if(data.headers.cookie) {
        ...
        data.sessionId = cookieParser.signedCookie(data.cookie['connect.sid'], self.conf.cookieSecret);
    }
    callback(null, true);
  });

  io.sockets.on('connection', function(socket) {
    var sessionStore = self.sessionStore,
    ...
    return sessionStore.get(sessionId, function(err, session) {
        if(session && session.passport.user) {
          return self.handleConnection(socket,
              crypto.createHash('md5').update(session.passport.user.username).digest('hex'),
...
Server.prototype.handleConnection = function(socket, user, userName) {
  var session = new Session(this, socket, user, userName);

  // XXX Possibly wrap socket events from inside Session
  // constructor, and do: session.on('create')
  // or session.on('create term').
  socket.on('create', function(conf, cols, rows, func) {
    return session.handleCreate(conf, cols, rows, func);
  });
  ...
  socket.on("list", function(f) {
    return session.listVMs(f);
  });

  socket.on("start", function(conf, on) {
    return session.handleVM("up", on, conf);
  });

  socket.on("status", function(conf, on) {
    return session.handleVM("status", on, conf);
  });

  socket.on("stop", function(conf, on) {
    return session.handleVM("halt", on, conf);
  });

  socket.on("createVM", function(conf, on) {
    return session.handleVM("init", on, conf);
  });

  socket.on("package", function(conf, on) {
    return session.handleVM("package", on, conf);
  });

  socket.on("getUser", function(callback) {
    callback(null, {"id": session.user, "name": session.userName});
  });
};
...
function Session(server, socket, user, userName) {
  this.server = server;
  this.socket = socket;
  this.terms = {};
...
Session.prototype.handleCreate = function(opts, cols, rows, func) {
  var self = this
    , terms = this.terms
  ...
  term = pty.fork(shell, shellArgs, {
    ...
    cols: cols,
    rows: rows,
    cwd: (conf.cwd || process.env.HOME) + path.sep + this.user + (opts ? path.sep + opts.name : '')
  });
...
Session.prototype.exeCmd = function(cmd, args, cb, opts) {
  var vmd = this.server.conf.cwd + path.sep + this.user;

  if(args.indexOf("init") != -1) {
    fs.mkdirSync(vmd + path.sep + opts.name);
    args.push(opts.name);
    var base = vmd + path.sep + opts.base + path.sep + opts.base + ".box";
    if(opts.base == "Ubuntu Server 14.04") {
        base = "https://cloud-images.ubuntu.com/vagrant/trusty/current/trusty-server-cloudimg-amd64-vagrant-disk1.box";
    } else if(opts.base == "Ubuntu Server 14.10") {
        base = "https://cloud-images.ubuntu.com/vagrant/utopic/current/utopic-server-cloudimg-amd64-vagrant-disk1.box";
    } else if(opts.base == "Ubuntu Server 15.04") {
        base = "https://cloud-images.ubuntu.com/vagrant/vivid/current/vivid-server-cloudimg-amd64-vagrant-disk1.box";
    }
    args.push(base);
  }

  if(args.indexOf("package") != -1) {
    if(fs.existsSync(vmd + path.sep + opts.name + path.sep + opts.name + ".box"))
        fs.unlinkSync(vmd + path.sep + opts.name + path.sep + opts.name + ".box");
    args.push("--output");
    args.push(opts.name + ".box");
  }

  var cwd = vmd + (opts ? path.sep + opts.name : ''),
      child = spawn(cmd, args, { cwd: cwd,
                                 env: { "VAGRANT_HOME": vmd + path.sep + ".vagrant" } }),
      self = this;

  child.stdout.on('data', function(data) {
    self.socket.emit('output', String(data));
  });
  child.stderr.on('data', function(data) {
    self.socket.emit('output', String(data));
  });
  child.on('close', function(exit) {
    cb(exit == 0 ? null : "Failed.");
  });
  child.on('error', function(error) { cb(String(error)); });
}

Session.prototype.handleVM = function(cmd, callback, opts) {
  this.exeCmd("vagrant", [cmd], callback, opts);
}

Session.prototype.listVMs = function(callback) {
  var that = this
    , vmd = this.server.conf.cwd + path.sep + that.user;

  if(!fs.existsSync(vmd)) {
    fs.mkdirSync(vmd);
  }

  var entries = fs.readdirSync(vmd)
    , vms = [];
  entries.forEach(function(e) {
    if(e != ".vagrant") {
        var boxed = fs.existsSync(vmd + path.sep + e + path.sep + e + ".box");
        vms.push({ "name": e, "boxed": boxed });
    }
  });
