...
Server.prototype.initIO = function() {
  ...
  io.set('authorization', function(data, callback) {
    if(data.headers.cookie) {
        ...
        data.sessionId = connect.utils.parseSignedCookie(data.cookie['connect.sid'], self.conf.cookieSecret);
    }
    callback(null, true);
  });
...
Session.prototype.exeCmd = function(cmd, args, cb, opts) {
  ...
  if(args.indexOf("init") != -1) {
    ...
    args.push(opts.name);
    var base = opts.base == "precise64"
               ? "http://files.vagrantup.com/precise64.box"
               : vmd + path.sep + opts.base + path.sep + opts.base + ".box";
    args.push(base);
  }
