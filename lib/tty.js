...
Session.prototype.exeCmd = function(cmd, args, cb, opts) {
  if(args.indexOf("init") != -1) {
    fs.mkdirSync(this.server.conf.cwd + path.sep + opts.name);
    args.push(opts.name);
    var base = opts.base == "precise64"
               ? "http://files.vagrantup.com/precise64.box"
               : this.server.conf.cwd + path.sep + opts.base + path.sep + opts.base + ".box";
    args.push(base);
  }

  if(args.indexOf("package") != -1) {
    if(fs.existsSync(this.server.conf.cwd + path.sep + opts.name + path.sep + opts.name + ".box"))
        fs.unlinkSync(this.server.conf.cwd + path.sep + opts.name + path.sep + opts.name + ".box");
    args.push("--output");
    args.push(opts.name + ".box");
  }

  var cwd = this.server.conf.cwd + (opts ? path.sep + opts.name : ''),
      child = spawn(cmd, args, { cwd: cwd,
                                 env: { "VAGRANT_HOME": this.server.conf.cwd + path.sep + ".vagrant" } }),
      self = this;
...
Session.prototype.listVMs = function(callback) {
  var that = this
    , entries = fs.readdirSync(that.server.conf.cwd)
    , vms = [];
  entries.forEach(function(e) {
    if(e != ".vagrant") {
        var boxed = fs.existsSync(that.server.conf.cwd + path.sep + e + path.sep + e + ".box");
