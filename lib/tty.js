...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on("start", function(f) {
    return exec("vagrant up", function(error, stdout, stderr) {
        return f(error ? stderr : null, stdout);
    });
  });

  socket.on("stop", function(f) {
    return exec("vagrant halt", function(error, stdout, stderr) {
        return f(error ? stderr : null, stdout);
    });
  });
};
