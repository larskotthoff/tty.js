...
Server.prototype.handleConnection = function(socket) {
  ...
  socket.on("status", function(on) {
    return session.handleVM("status", on);
  });
...
Session.prototype.exeCmd = function(cmd, args, cb) {
...
Session.prototype.handleVM = function(cmd, callback) {
  this.exeCmd("vagrant", [cmd], callback);
}
